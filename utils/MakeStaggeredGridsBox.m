function [grids,filtering,par] = MakeStaggeredGridsBox(par)
	...
	%inner
	uxinit = (limits(1)+h/2:par.h:limits(2)-h/2)';
	uyinit = (limits(3)+h:par.h:limits(4)-h)';
	nx = numel(uxinit);
	nym1 = numel(uyinit);
	...
	for i=1:5	
		if((i==2 || i==3))
			uxlimcoords(i) = uxlimcoords(i)-h/2;
		else
			uxlimcoords(i) = uxlimcoords(i)+h/2;
		end
		
		if(i>1 && i<4)
			uylimcoords(i) = uylimcoords(i)-h;
		else
			uylimcoords(i) = uylimcoords(i)+h;
		end
	end
	[ugrids,ufiltering] = createGridsInner(uxinit,uyinit,nx,nym1,uxlimcoords,uylimcoords,h,par);

	%outer
	uxinit = (limits(1)-h/2:par.h:limits(2)+h/2)';
	uyinit = (limits(3):par.h:limits(4))';
	nxp2 = numel(uxinit);
	nyp1 = numel(uyinit);
	uxlimcoords = xlimcoords;
	uylimcoords = ylimcoords;
	...
	[ugrids,ufiltering] = createGridsOuter(uxinit,uyinit,nxp2,nyp1,uxlimcoords,uylimcoords,h,ugrids,ufiltering,par);
	...
	%inner
	vxinit = (limits(1)+h:par.h:limits(2)-h)';
	vyinit = (limits(3)+h/2:par.h:limits(4)-h/2)';
	nxm1 = numel(vxinit);
	ny = numel(vyinit);
	...
	for i=1:5
		if(i==2 || i==3)
			vxlimcoords(i) = vxlimcoords(i)-h;
		else
			vxlimcoords(i) = vxlimcoords(i)+h;
		end
		
		if(i>1 && i<4)
			vylimcoords(i) = vylimcoords(i)-h/2;
		else
			vylimcoords(i) = vylimcoords(i)+h/2;
		end
	end
	[vgrids,vfiltering] = createGridsInner(vxinit,vyinit,nxm1,ny,vxlimcoords,vylimcoords,h,par);	

	%outer
	vxinit = (limits(1):par.h:limits(2))';
	vyinit = (limits(3)-h/2:par.h:limits(4)+h/2)';
	nxp1 = numel(vxinit);
	nyp2 = numel(vyinit);
	vxlimcoords = xlimcoords;
	...
	[vgrids,vfiltering] = createGridsOuter(vxinit,vyinit,nxp1,nyp2,vxlimcoords,vylimcoords,h,vgrids,vfiltering,par);	
